// https://github.com/ZGGSONG/LangIndicator/blob/main/src/LangIndicator/ShortcutUtilities.cs
...
public class ShortcutUtilities
{
    ...
    private static string? GetAppPathViaShortCut(string shortCutPath)
    {
        try
        {
            ...
            file.GetPath(sb, sb.Capacity, nint.Zero, 2);
            return sb.ToString();
        }
        catch
        {
    ...
    {
        void GetPath([Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder pszFile, int cchMaxPath, nint pfd,
            int fFlags);
